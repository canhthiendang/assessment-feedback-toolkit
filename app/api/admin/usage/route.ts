...
export async function GET() {
  ...
  try {
    ...
    const result = await db.prepare(`SELECT month, academic_period AS academicPeriod, institution, faculty, department, module_level AS moduleLevel, output_type AS outputType, stage, workflow, assessment_profile AS assessmentProfile, assessment_format AS assessmentFormat, COUNT(*) AS events FROM usage_events GROUP BY month, academic_period, institution, faculty, department, module_level, output_type, stage, workflow, assessment_profile, assessment_format ORDER BY month DESC, institution, faculty, department`).all();
