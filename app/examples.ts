...
export function aiPrompt(stage: DocumentStage, format: AssessmentFormat | '', product: 'guide' | 'slides' = 'guide') {
  const formatText = format ? formatLabels[format] : 'the assessment format supported by the source material';
  const shared = `You are helping a university module leader create an editable student-facing document for an undergraduate economics or management module. Before drafting, ask the user to upload or paste approved source documents wherever possible: the module specification, syllabus, module handbook, assessment brief, assessment rubric, learning outcomes and any other approved module documentation. Explain that fuller source material improves accuracy and reduces manual editing.

Use supplied sources for all module-specific facts. Extract learning outcomes only when they appear in the supplied material and reproduce their meaning accurately. Never invent learning outcomes, assessment requirements, module content, dates, rules, links, grade boundaries, marking-band labels, statistics or cohort results. Leave unsupported module-specific values empty and list every material gap in missingInformation. You may offer a general pedagogical suggestion only for a missing explanatory field such as assessment rationale, preparation, feedback use or employability. Prefix it with "[AI suggestion - verify]" and ask the module leader to confirm, edit or replace it. Never present a suggestion as an approved module fact.

Use concise, constructive language and a feedback-for-learning approach: help students recognise quality, diagnose their next step and transfer learning to future work. Do not include student names, IDs, individual scripts, raw marks, named comments or identifiable student information.
...
Return ONLY one valid JSON object with string values and no markdown.`;
  if (stage === 'before' && product === 'slides') return `${shared}\n\nCreate content for a three-slide Assessment and Feedback Slides pilot for ${formatText}, plus an accessible narrative companion document. Preserve this order: (1) assessment components and rationale alongside plain-English learning outcomes; (2) marking criteria and fairness/clarity safeguards alongside feedback opportunities and how students can use feedback; (3) two to four relevant skills and employability benefits, each linked to a concrete assessment activity. Also provide preparation, common pitfalls, worked examples and support routes for the companion document. Use exactly these keys:\n${JSON.stringify({ moduleCode:'', moduleTitle:'', level:'', faculty:'', department:'', moduleLeader:'', academicYear:'', teachingPeriod:'', assessmentProfile:'', assessmentFormat:format, assessmentStructure:'', assessmentPurpose:'', learningOutcomes:'', criteria:'', fairnessAndClarity:'', preparation:'', workedExamples:'', commonPitfalls:'', assessmentRequirements:'', feedbackAvailable:'', usingFeedback:'', supportRoutes:'', skillsEmployability:'', keatsLocation:'', missingInformation:'' }, null, 2)}\n\nUse assessmentProfile values exam, mixed or coursework. Keep slide-ready fields concise. In learningOutcomes, use only supplied outcomes and translate them into plain English without adding new outcomes. Explain alignment only where the supplied outcomes and assessment evidence support it. In skillsEmployability, connect each claimed skill to what students actually do in this assessment. Make clear that all [AI suggestion - verify] text requires academic confirmation.`;
  if (stage === 'before') return `${shared}\n\nCreate an Assessment and Feedback Guide for ${formatText}. Use tiered illustrations in workedExamples and explain how the supplied criteria distinguish levels of performance in criteria. Use exactly these keys:\n${JSON.stringify({ moduleCode:'', moduleTitle:'', level:'', faculty:'', department:'', moduleLeader:'', academicYear:'', teachingPeriod:'', assessmentProfile:'', assessmentFormat:format, assessmentStructure:'', assessmentPurpose:'', expectations:'', criteria:'', preparation:'', workedExamples:'', commonPitfalls:'', assessmentRequirements:'', feedbackAvailable:'', usingFeedback:'', supportRoutes:'', keatsLocation:'', missingInformation:'' }, null, 2)}\n\nUse assessmentProfile values exam, mixed or coursework. Explain the educational purpose without claiming the format is inherently superior or valuable merely because it restricts AI. Leave unsupported module facts empty and record gaps in missingInformation.`;
  return `${shared}\n\nCreate a cohort-level General Feedback document for ${formatText}. In addition to the approved syllabus, learning outcomes, marking guide and rubric, use only checked, anonymised aggregate cohort themes. Never infer or calculate performance statistics; include a numeric pattern only when explicitly supplied and checked. Use tiered illustrations in improvedApproaches and explain how the supplied criteria distinguished performance in criteriaApplication. Use exactly these keys:\n${JSON.stringify({ moduleCode:'', moduleTitle:'', level:'', faculty:'', department:'', moduleLeader:'', academicYear:'', teachingPeriod:'', assessmentProfile:'', assessmentFormat:format, cohortContext:'', evaluatedLearning:'', performancePatterns:'', cohortStrengths:'', improvementAreas:'', criteriaApplication:'', improvedApproaches:'', futureUse:'', debriefSupport:'', keatsLocation:'', missingInformation:'' }, null, 2)}\n\nUse assessmentProfile values exam, mixed or coursework. Make clear that cohort feedback complements rather than replaces individual feedback. Leave unsupported module or cohort facts empty and record gaps in missingInformation.`;
}
...
const slidesAiSample = {
  ...beforeAiSample,
  learningOutcomes: 'Explain how incentives shape market behaviour. Apply an appropriate microeconomic model to an unfamiliar problem. Interpret analytical or quantitative results and reach a justified conclusion.',
  fairnessAndClarity: 'Students receive the approved rubric and an assessment briefing. Practice questions and a worked approach clarify the expected reasoning. The module team applies the published moderation process.',
  skillsEmployability: 'Critical thinking: compare plausible models and justify the most appropriate one. Quantitative reasoning: show and interpret calculations rather than report an answer alone. Communication: explain economic reasoning clearly for the intended audience. AI literacy: evaluate tool-assisted claims against theory, evidence and the assessment rules.',
  missingInformation: 'Fictional example only. Confirm the exact assessment weighting, approved learning outcome wording, permitted resources, feedback arrangements and moderation statement from current module documents.'
};

export function sampleAiResponse(stage: DocumentStage, product: 'guide' | 'slides' = 'guide') {
  if (stage === 'before' && product === 'slides') return JSON.stringify(slidesAiSample, null, 2);
